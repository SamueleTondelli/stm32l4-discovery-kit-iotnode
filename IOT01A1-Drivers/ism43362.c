...
#define OK_MSG "\r\nOK\r\n"
#define RET_IF_NOT_OK(ret) if(ret != Ok) { return ret; }
//#define USART1_LOG
...
ISM43362_RET ism43362_reset_module() {
  ...
  data_ready = 0;
  if (init_cursor[0] != 0x15 || init_cursor[1] != 0x15 || init_cursor[2] != '\r' ||
...
ISM43362_RET ism43362_transmit_buffer(const uint8_t* tr_buffer, size_t tr_size, uint8_t* resp, size_t resp_buff_len, size_t* resp_len) {
...
ISM43362_RET ism43362_join_network(const JoinWifiConfig* conf) {
  ...
  snprintf(cmd, cmd_size, "C8=%d.%d.%d.%d\r\n", conf->gateway[0], conf->gateway[1], conf->gateway[2], conf->gateway[3]);
  ret = ism43362_execute_cmd(cmd, buff, buff_size, &resp_len);
  RET_IF_NOT_OK(ret);

  snprintf(cmd, cmd_size, "C9=%d.%d.%d.%d\r\n", conf->primary_dns[0], conf->primary_dns[1], conf->primary_dns[2], conf->primary_dns[3]);
  ret = ism43362_execute_cmd(cmd, buff, buff_size, &resp_len);
  RET_IF_NOT_OK(ret);

  snprintf(cmd, cmd_size, "CA=%d.%d.%d.%d\r\n", conf->secondary_dns[0], conf->secondary_dns[1], conf->secondary_dns[2], conf->secondary_dns[3]);
  ret = ism43362_execute_cmd(cmd, buff, buff_size, &resp_len);
  RET_IF_NOT_OK(ret);
...
ISM43362_RET ism43362_read_wifi_config(JoinWifiConfig* conf) {
  ...
  int status;
  sscanf(buff, "\r\n%[^,],%[^,],%d,%d,%d,%[^,],%[^,],%[^,],%[^,],%[^,],%d,%d,%d,%[^,],%d\r\nOK\r\n",
                conf->ssid, conf->password, conf->security, &dhcp, &ip_v, ip_addr, mask, gw, dns1, dns2, &conf->join_retry_count,
...
ISM43362_RET ism43362_get_remote(WifiRemote* remote) {
  ...
  int local_port;
  uint8_t host_ip[4];
  sscanf(buff, "\r\n%d,%d.%d.%d.%d,%d,%d.%d.%d.%d,%d,", &proto, remote->ip, remote->ip+1, remote->ip+2, remote->ip+3, &local_port,
    host_ip, host_ip+1, host_ip+2, host_ip+3, &remote->port);
  return Ok;
}

WifiClientConfig ism43362_get_default_client_config() {
  WifiClientConfig conf = {
    .s = SOCKET_0,
    .protocol = TCP,
...
ISM43362_RET ism43362_start_wifi_client(const WifiClientConfig *client) {
  ...
  snprintf(cmd, sizeof(cmd), "P3=%d.%d.%d.%d\r\n", client->remote.ip[0], client->remote.ip[1], client->remote.ip[2], client->remote.ip[3]);
  ret = ism43362_execute_cmd(cmd, buff, buff_size, &resp_len);
  RET_IF_NOT_OK(ret);
...
WifiTcpServerConfig ism43362_get_default_tcp_server_config() {
  WifiTcpServerConfig conf = {
    .base_conf = ism43362_get_default_base_server_config(),
    .listen_backlogs = 1,
...
ISM43362_RET ism43362_check_tcp_server_connection(RemoteTcpConnection* conn) {
  ...
  conn->connected = true;
  sscanf(buff, "\r\n[SOMA][TCP SVR] Accepted %d.%d.%d.%d:%d", conn->remote.ip, conn->remote.ip+1, conn->remote.ip+2,
      conn->remote.ip+3, &conn->remote.port);
